...
class Affine_Inv_Evaluator(Evaluator):
    ...
    def compute_affine_inv_eval_metrics(self, gt_depth, pred_depth, mask=None):
        ...
        if len(pred_depth.shape) == 4:
            pred_depth, gt_depth, mask = pred_depth.squeeze(1), gt_depth.squeeze(1), mask.squeeze(1)
        # # depth -> disp
        # gt_disp = gt_depth.clone()  # allocate new memory
        # gt_disp[gt_disp != 0] = 1 / gt_disp[gt_disp != 0]
        # scale, shift = compute_scale_and_shift(pred_depth, gt_disp, mask) # scale shift on disp domain
        # pred_depth = (scale.view(-1, 1, 1) * pred_depth + shift.view(-1, 1, 1)) # not call by reference, new tensor
        #
        # # disp -> depth
        # pred_depth[pred_depth != 0] = 1 / pred_depth[pred_depth != 0]

        pred_depth = torch.clip(pred_depth, 0, self.max_depth)

        abs_, abs_rel, sq_rel, rms, rms_log, log10, a1, a2, a3 = \
            compute_depth_metrics(gt_depth, pred_depth, mask, max_depth=self.max_depth)
