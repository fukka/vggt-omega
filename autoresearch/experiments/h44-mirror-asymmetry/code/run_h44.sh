#!/usr/bin/env bash
# H44 — mirror test. $1 = deltas (must include 0), $2 = tag, $3 = GPU.
set -u
...
OUT=autoresearch/experiments/h44-mirror-asymmetry/results
mkdir -p "$OUT"
...
for s in $SEQS; do
  ...
  if [ -s "$f" ]; then echo "[h44] skip $tag ($TAG)"; continue; fi
  echo "[h44] === $TAG $tag ==="
  python autoresearch/experiments/h44-mirror-asymmetry/code/mirror_curve.py \
    --seq "$s" --calib "$CALIB" --deltas="$DELTAS" --max-frames 20 \
    --models da3:small,vggt --out "$f" || echo "[h44] FAILED $TAG $tag"
done
echo "[h44] $TAG done: $(ls "$OUT"/${TAG}_*.json 2>/dev/null | wc -l) files"
