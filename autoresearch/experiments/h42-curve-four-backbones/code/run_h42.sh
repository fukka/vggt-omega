#!/usr/bin/env bash
# H42 — residual-roll curve for the other three backbones.
# $1 = deltas (must include 0), $2 = tag, $3 = GPU.
set -u
...
OUT=autoresearch/experiments/h42-curve-four-backbones/results
mkdir -p "$OUT"
...
for s in $SEQS; do
  ...
  if [ -s "$f" ]; then echo "[h42] skip $tag ($TAG)"; continue; fi
  echo "[h42] === $TAG $tag ==="
  python autoresearch/experiments/h41-residual-curve/code/residual_curve.py \
    --seq "$s" --calib "$CALIB" --deltas="$DELTAS" --max-frames 20 \
    --models da3:large,vggt,vggt_omega --out "$f" \
    || echo "[h42] FAILED $TAG $tag"
done
echo "[h42] $TAG done: $(ls "$OUT"/${TAG}_*.json 2>/dev/null | wc -l) files"
