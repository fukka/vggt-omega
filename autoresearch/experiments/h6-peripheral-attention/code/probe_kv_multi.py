"""H6.2 probe: training-free multi-frame rim-KV. Protocol addendum
2026-08-19b. Variants: rim(t-1) / +rim(t-2) / +rim(t-2)+rim(t-4).

Usage (repo root):
    <venv>/bin/python autoresearch/experiments/h6-peripheral-attention/code/probe_kv_multi.py \
        --seq <seq dir> --module <module_last.pt> --out results/probe_kv_multi_<seq>.json
"""
...
VARIANTS = {"t1": [1], "t1_t2": [1, 2], "t1_t2_t4": [1, 2, 4]}


def apply_multi(module, feats_t, prev_list, rim_mask, kv_mask):
    ...
    prev = torch.cat([fp[-1][0][:, 0, kv_idx, :] for fp in prev_list], dim=1)
...
def main(argv=None) -> None:
    ...
    kv_rim = (theta_p <= float(cam.theta_max)) \
        & (theta_p > math.radians(rim_deg))
    with torch.no_grad():
    ...
    feats_by_pos = {}
    for pos, n in enumerate(s.frames):
        with torch.no_grad():
            feats, _ = net.backbone(s.src.image(n)[None, None]
                                    .to(args.device), cam_token=None,
                                    export_feat_layers=[])
        feats_by_pos[pos] = feats
        max_lag = max(max(v) for v in VARIANTS.values())
        if pos >= max_lag:   # every variant has its full context
            gr = s.gt_range(n, cos_t).numpy()
            for k, lags in VARIANTS.items():
                prev_list = [feats_by_pos[pos - l] for l in lags]
                with torch.no_grad():
                    d = depth_from(apply_multi(module, feats, prev_list,
                                               rim, kv_rim))
        ...
        feats_by_pos = {q: f for q, f in feats_by_pos.items()
                        if q > pos - 5}
    ...
    for k in VARIANTS:
        ...
        for zname, cells in zones_def.items():
            ...
            z[zname] = float((np.array([table[i, j] for i, j in cells])
                              * wg).sum() / wg.sum())
        report[k] = z
        print(f"kv={k:10s} near_rim {z['near_rim(<=2m,>=38deg)']:.3f}  "
              f"center {z['center(<=11deg)']:.3f}  far {z['far(>=3m)']:.3f}")
