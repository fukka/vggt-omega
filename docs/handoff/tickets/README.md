...
| # | | owner | queue |
|---|---|---|---|
| [028](028-da3-row-and-the-papers-span.md) | **DA3's `raytun3r` row (now unblocked), and the whole table re-evaluated at stride 60** — the span ticket 10 identified as the paper's protocol, so every cell lines up with Tab. 2's absolute numbers. One fit (DA3), the rest is eval. | gpu | with the 026 re-check, same session |
| [026](026-da3-rope-n-prefix-and-checkpointing-drift.md) (#26) | **Fixed on cpu, needs the 5-min GPU re-check.** Both items were misdiagnosed: DA3 passes positions *positionally* but its global attention is spatially unencoded by design (`pos_nodiff`) — the hook now skips those calls; and the checkpointing "drift" was the un-pinned matcher→MAGSAC pipeline, not checkpointing (bit-identical with windows held fixed, test-pinned). Re-check: real-package pytest + the `--windows-cache` A/B. | gpu | 5 min, before/with 028 |
