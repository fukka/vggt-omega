...
#: Incidence-angle bin edges for the 110 deg rectified pinhole.
#:
#: The render's *geometry* reaches 55 deg at the middle of an edge and 63.65 deg
#: into a corner, but the corners are never imaged: the fisheye's cone is
#: inscribed in the square render, so ``rectified_valid_mask.png`` covers ~86 %
#: of the frame and cuts everything beyond it. Measured over all four sample
#: takes, the widest ray the mask admits is 56.6-57.0 deg and the widest a GT
#: point actually sits at is **56.99 deg** — so edges running to 63.7 would
#: carry a bin that is empty by construction, in every run, forever.
#:
#: The top edge is therefore 58: past every ray the data contains, with headroom
#: for a take whose calibration reaches slightly further, and no dead bin. This
#: is the same rule as the ADT edges' 55 (just past the 54.83 deg Aria cone).
#: Points beyond the top edge would still count in ``overall`` while appearing
#: in no bin, so the headroom is not cosmetic.
THETA_EDGES: Tuple[float, ...] = (0.0, 10.0, 20.0, 30.0, 40.0, 50.0, 58.0)
...
_CTX_CACHE: Dict[str, Tuple[Rectification, Optional[np.ndarray]]] = {}


def context_for(npz_path: str) -> Tuple[Rectification, Optional[np.ndarray]]:
    """``(rectification, rectified_valid_mask)`` for the take owning ``npz_path``.

    Derived from the path — ``<take>/sparse_depth/<clip>.npz`` — rather than
    carried through the manifest, so a run resumed from a saved ``--manifest``
    reads exactly the same geometry as the run that wrote it, out of the take's
    own ``meta.json``. Memoised per take: a take is one rectification and one
    mask across all of its clips.
    """
    take_dir = os.path.dirname(os.path.dirname(os.path.abspath(npz_path)))
    if take_dir not in _CTX_CACHE:
        meta_path = os.path.join(take_dir, "meta.json")
        if not os.path.isfile(meta_path):
            raise SystemExit(f"[fovbench] {npz_path!r} has no meta.json beside "
                             f"its take at {take_dir!r}")
        with open(meta_path) as fh:
            meta = json.load(fh)
        _CTX_CACHE[take_dir] = (
            Rectification.from_meta(meta),
            load_valid_mask(os.path.join(take_dir, "rectified_valid_mask.png")))
    return _CTX_CACHE[take_dir]


def resize_for_model(rgb: np.ndarray, out_size: int) -> np.ndarray:
    """The 896 frame at a model's own token grid, or untouched if it already is.

    Same rule as the ADT path: a view goes to each network at its native size
    (518 for VGGT/DA3, 512 for VGGT-Omega) so nothing is resampled between the
    view and the network. Here the frame arrives fixed at 896, so the resample
    happens once, on the way in, and the GT points are mapped into the model's
    grid afterwards by :func:`sample_prediction` rather than the prediction
    being blown back up.
    """
...
def prediction_at_points(raw: np.ndarray, pts: FramePoints) -> np.ndarray:
    """A model's answer as one depth per GT point, however it chose to answer.

    Every real model here takes an image and returns a depth **map**, which is
    gathered at the point list by :func:`sample_prediction`. The analytic
    stand-in (``--models analytic``) instead reads the ``gt_z``/``theta_deg`` it
    is handed and answers elementwise, so on this dataset it answers *per point*
    — which is what lets a laptop run inject a known ``+0.6 theta^2`` bias and
    read it back without any weights. Dispatching on the shape of the answer
    keeps that difference out of the driver.

    The gather itself is therefore not exercised by an analytic run, and is
    pinned separately by
    ``tests/test_egosynth.py::test_the_gather_reads_the_pixel_the_point_names``.
    """
    raw = np.asarray(raw)
    if raw.ndim == 1 and raw.shape[0] == len(pts):
        return raw.astype(np.float32)
    return sample_prediction(raw, pts)
    ...
    def profile(self, edges_by_axis: Dict[str, Sequence[float]],
        ...
        for axis, edges in edges_by_axis.items():
            ...
            for i, (lo, hi) in enumerate(zip(edges[:-1], edges[1:])):
                ...
                # The anchored pool carries NaN for frames that could not
                # populate the shared anchor band; that is the point, and
                # ``raw_scale_ratio``'s own ``p > 1e-8`` drops them. Comparing
                # against NaN is well-defined here and merely noisy, so the
                # warning is silenced rather than the NaN replaced — a 0.0 fill
                # would read as a prediction that happened to be zero.
                with np.errstate(invalid="ignore"):
                    met["anchored_ratio"] = (
                        G.raw_scale_ratio(anchored, gt, m)
